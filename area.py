...
class Area():
    ...
    def split(self, percentage, direction, inplace=True, new_category=Category.GARDEN):
        ...
        res = path.intersection(self._polygon.boundary)
        try:
            res = list(res)[-1]  # we may have more than one intersection
        except:
            pass
        pts = self._polygon.boundary.coords
        for pt1, pt2 in zip(pts[:-1], pts[1:]):
            if LineString((pt1, pt2)).distance(res) < 1E-6:
                break
        pt1 = np.array(pt1)
        pt2 = np.array(pt2)
        ...
        width = house_area / dist  # ok only if we have a rectangle
        res = [self._polygon,]
